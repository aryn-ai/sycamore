...
def make_openai_call(messages, model_id="gpt-4"):
    response = client.chat.completions.create(model=model_id, messages=messages, temperature=0.0, stream=False)
...
def simplify_answer():
    ...
    summarize_prompt = """
You are a post processor for a question answering system. Your job is to modify a \
system generated answer for a user question, based on the type of question it is.
...
1. If the question is a process lookup and there is a valid answer, say "The answer can be found \
in search results [{citations}]", followed by a 1 sentence summary of the answer. Do not use more than 1 sentence.
...
If citations are surrounded by a dollar sign and curly braces, e.g. [${1}], change it to only be a number \
surrounded by square brackets, i.e. [1]

Examples:
Question: what is the part number I need for a service kit for my Reba RL A5?

System generated answer:
The answer can be found in search result(s) [1]. The part number for the service kit for a Reba RL A5 is \
00.4315.032.650.
...
5. If your target sag percentage is not achieved, spring preload adjustment and/or coil spring replacement must \
be performed.

Answer:
The answer can be found in search result [4]. To adjust the sag on your Super Deluxe Coil, you will need to install \
a coil spring and spring retainer, followed by some additional adjustments.
...
def interpret_os_result():
    ...
    summarize_prompt = """
The following is a result from an opensearch query which was correctly generated to answer a user question. \
Given a user question and this result, synthesize an answer
doc_count does not mean number of crashes but instead number of references in the indexed documents, do not \
reference the doc_count field.
