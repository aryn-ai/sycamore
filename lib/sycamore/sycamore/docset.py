        ...
        **resource_args,
        ...
        def process_doc(doc: Document) -> bool:
            ...
            if date:
                ...
                value_comp = parser.parse(value).replace(tzinfo=None)
                if start and not isinstance(start, str):
                    raise ValueError("start must be a string for date filtering")
                start_comp = parser.parse(start).replace(tzinfo=None) if start else None
                if end and not isinstance(end, str):
                    raise ValueError("end must be a string for date filtering")
                end_comp = parser.parse(end).replace(tzinfo=None) if end else None
            else:
                value_comp = value
                start_comp = start
                end_comp = end
