...
def subtask_to_qa_datapoint(question: str, filters: dict[str, str], code: str) -> dict[str, Any]:
    question = question.format(**filters, year1=int(filters["year"])-1, year2=int(filters["year"])-2)

    document = EvaluationDataPoint()
    document.question = question + "Return only the code "+ code +" alongside the dollar amount found and no additional information."
...
def executor(question, filters, filepath, index, query_executor, os_config):
    ...
    final_q = "Use the given formula and dictionary of values to answer the question that follows. Note that values in parentheses are negative. "
    ...
    if len(task_formulas[task_id]) > 1:
        final_q += task_formulas[task_id][-1].format(**filters, year1=filters["year"]-1, year2=filters["year"]-2)
