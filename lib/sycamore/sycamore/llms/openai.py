...
class OpenAI(LLM):
    ...
    def generate(self, *, prompt: RenderedPrompt, llm_kwargs: Optional[dict] = None, model: Optional[LLMModel] = None) -> str:
        ...
        if prompt.response_format is not None:
            ret = self._generate_using_openai_structured(model, prompt, llm_kwargs)
        else:
            ret = self._generate_using_openai(model, prompt, llm_kwargs)
    ...
    async def generate_async(self, *, prompt: RenderedPrompt, llm_kwargs: Optional[dict] = None, model: Optional[LLMModel] = None) -> str:
        ...
        while not done:
            try:
                if prompt.response_format is not None:
                    ret = await self._generate_awaitable_using_openai_structured(model, prompt, llm_kwargs)
                else:
                    ret = await self._generate_awaitable_using_openai(model, prompt, llm_kwargs)
                done = True
    ...
    async def _generate_awaitable_using_openai(self, model: str, prompt: RenderedPrompt, llm_kwargs: Optional[dict]) -> str:
        kwargs = self._get_generate_kwargs(prompt, llm_kwargs)
        starttime = datetime.now()
        if self.is_chat_mode():
    ...
    def generate_batch(self, *, prompts: list[RenderedPrompt], llm_kwargs: Optional[dict] = None, model: Optional[LLMModel] = None) -> list[str]:
        ...
        if batch.output_file_id:
            ...
            for rs, call in zip(responses.iter_lines(), calls):
                ...
                kws = call["body"]
                self.add_llm_metadata(kws, response_text, wall_latency, ct, pt, model=model)
                cache_hits[id] = response_text
                self._llm_cache_set(prompts[id], llm_kwargs, response_text, model=model)
            return cache_hits
