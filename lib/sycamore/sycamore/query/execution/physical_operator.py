...
class PhysicalOperator:
    ...
    Args:
        logical_node (Node): The logical query plan node to execute. Contains runtime params based on type.
        query_id (str): Query id
        inputs (List[Any]): List of inputs required to execute the node. Varies based on node type.
    """

    def __init__(self, logical_node: Node, query_id: str, inputs: Optional[List[Any]] = None) -> None:
        super().__init__()
...
class MathOperator(PhysicalOperator):
    ...
    def script(self, input_var: Optional[str] = None, output_var: Optional[str] = None) -> Tuple[str, List[str]]:
        assert isinstance(self.logical_node, Math)
        assert len(self.logical_node.inputs) == 2
        operator = self.logical_node.operation
        result = f"""
{output_var or get_var_name(self.logical_node)} = math_operation(
    val1={input_var or get_var_name(self.logical_node.input_nodes()[0])},
    val2={input_var or get_var_name(self.logical_node.input_nodes()[1])},
    operator='{operator}'
)
"""
