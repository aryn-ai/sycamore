...
class Count(LogicalOperator):
    ...
    def description() -> str:
        return """
        **Count**: Determines the length of a particular data table. Parameters are *description*,
        ...
        - *input* is a list of operation ids that this operation depends on. For this operation,
            *input* should only contain one id of an operation that returns a data table (len(input) == 1).
