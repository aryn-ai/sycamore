...
class LlmExtract(LogicalOperator):
    ...
    def description() -> str:
        return """
        *LlmExtract**: Adds a new field to the inputted data table based on extracting information from an existing field. 
        Parameters are *description*, *question*, *field*, *newField*, *format*, *input*, and *id*. Returns a data table.
        ...
        - *question* is the prompt to the LLM for creating the new field
        ...
        - *discrete* is a Boolean. It is true ONLY if *newField* has a known finite number of possible values (e.g. number, letter, continent, color). It
        is ALWAYS false otherwise (any free text outputs).
