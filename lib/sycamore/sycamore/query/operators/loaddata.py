...
class LoadData(LogicalOperator):
    ...
    def description() -> str:
        return """
        **LoadData**: Loads data from a specified index. Parameters are *description*, *index*, *query*, and *id*. Returns a data table.
        ...
        - *query* is the initial query to search for when loading data (so that only the relevant data is used)
