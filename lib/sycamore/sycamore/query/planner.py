...
class QueryPlanNode(BaseModel):
    ...
    class Config:
        ...
        # Allow additional fields in the JSON representation.
        extra = "allow"
...
guidelines when generating a plan:
        ...
        1. Return your answer as a standard JSON object containing a single key, "nodes".
            "nodes" should be a list of JSON objects with one query operator per entry. Make
            sure to include each operation as a separate step.
        2. Do not return any information except the standard JSON object. This means not repeating
           the question or providing any text outside the json block.
...
def process_json_plan(
    ...
    if isinstance(plan, str):
        parsed_plan = QueryPlan(**extract_json(plan))
    else:
        parsed_plan = QueryPlan(nodes=plan)

    nodes: MutableMapping[int, LogicalOperator] = {}
    downstream_dependencies: Dict[int, List[int]] = {}

    # 1. Build nodes
    for step in parsed_plan.nodes:
        node_id = step.node_id
        cls = classes.get(step.operator_name)
        if cls is None:
            raise ValueError(f"Operator {step.operator_name} not found")
        if cls not in operators:
            raise ValueError(f"Operator {step.operator_name} is not a valid operator")
        try:
            node = cls(**step.model_dump())
            nodes[node_id] = node
        except Exception as e:
            logging.error(f"Error creating node {node_id} of type {step.operator_name}: {e}\nPlan is:\n{plan}")
            raise ValueError(f"Error creating node {node_id} of type {step.operator_name}: {e}") from e
...
EXAMPLE_NTSB_SCHEMA: OpenSearchSchema = {
    "properties.path": OpenSearchSchemaField(
        type="str", samples={"/docs/incident1.pdf", "/docs/incident2.pdf", "/docs/incident3.pdf"}
    ),
    "properties.entity.date": OpenSearchSchemaField(type="date", samples={"2023-07-01", "2024-09-01"}),
    "properties.entity.accidentNumber": OpenSearchSchemaField(type="str", samples={"1234", "5678", "91011"}),
    "properties.entity.location": OpenSearchSchemaField(
        type="str", samples={"Atlanta, Georgia", "Miami, Florida", "San Diego, California"}
    ),
    "properties.entity.aircraft": OpenSearchSchemaField(type="str", samples={"Cessna", "Boeing", "Airbus"}),
    "properties.entity.city": OpenSearchSchemaField(type="str", samples={"Atlanta", "Savannah", "Augusta"}),
    "text_representation": OpenSearchSchemaField(type="str", samples={"Can be assumed to have all other details"}),
}

EXAMPLE_FINANCIAL_SCHEMA: OpenSearchSchema = {
    "properties.path": OpenSearchSchemaField(type="str", samples={"doc1.pdf", "doc2.pdf", "doc3.pdf"}),
    "properties.entity.date": OpenSearchSchemaField(type="str", samples={"2022-01-01", "2022-12-31", "2023-01-01"}),
    "properties.entity.revenue": OpenSearchSchemaField(type="float", samples={"1000000.0", "2000000.0", "3000000.0"}),
    "properties.entity.firmName": OpenSearchSchemaField(
        type="str",
        samples={"Dewey, Cheatem, and Howe", "Saul Goodman & Associates", "Wolfram & Hart"},
    ),
    "text_representation": OpenSearchSchemaField(type="str", samples={"Can be assumed to have all other details"}),
}
...
PLANNER_EXAMPLES = [
    PlannerExample(
        ...
        data_schema=EXAMPLE_NTSB_SCHEMA,
        plan=QueryPlan(
            nodes=[
                QueryPlanNode(
 ...
 .         number containing 'K1234N' that occurred in Georgia.""",
    ...
    PlannerExample(
        ...
        plan=QueryPlan(
            nodes=[
                ...
                QueryPlanNode(
                    operator_name="LlmExtractEntity",
    ...
    PlannerExample(
        ...
        data_schema=EXAMPLE_FINANCIAL_SCHEMA,
        plan=QueryPlan(
            nodes=[
                QueryPlanNode(
                    operator_name="QueryDatabase",
...
class LlmPlanner:
    ...
    def make_examples_prompt(self) -> str:
        ...
        for example in self._examples:
            ...
            schema_prompt = self.make_schema_prompt(example.data_schema)
    ...
    def generate_from_llm(self, question: str) -> Tuple[Dict[str, List], str]:
