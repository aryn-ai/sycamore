...
class BooleanExpValidator(PropertyValidator):
    ...
    def validate_property(self, propval: Any, context: Optional[dict] = None) -> tuple[bool, Any]:
        """Validate the property value against the boolean expression.

        Args:
            propval: The value to validate
            context: Optional context dict containing other field values for cross-field validation

        Returns:
            tuple: (is_valid, processed_value)
        """
        try:
            evaluator = BooleanExpressionEvaluator(self.expression)
            is_valid = evaluator.evaluate(propval, context or {})
            return is_valid, propval
        except Exception as e:
            logger.warning(f"Boolean expression validation failed: {e}")
            return False, propval
...
class BooleanExpressionEvaluator:
    ...
    def evaluate(self, value: Any, context: dict) -> bool:
        tokens = self._tokenize(self.expression)
        return self._evaluate_tokens(tokens, value, context)

    def _tokenize(self, expr: str) -> list[str]:
        pattern = r"(\band\b|\bor\b|\bnot\b|\bis\b|\bnull\b|>=|<=|==|!=|>|<|\(|\)|len\(\)|[a-zA-Z_][a-zA-Z0-9_]*|[0-9]+(?:\.[0-9]+)?)"
        tokens = re.findall(pattern, expr)
        return [token for token in tokens if token.strip()]

    def _evaluate_tokens(self, tokens: list[str], value: Any, context: dict) -> bool:
        if not tokens:
            return True

        return self._evaluate_expression(tokens, value, context)

    def _evaluate_expression(self, tokens: list[str], value: Any, context: dict) -> bool:
        ...
        def parse_comparison():
            if pos[0] < len(tokens) and tokens[pos[0]] in [">", ">=", "<", "<=", "==", "!="]:
                ...
                pos[0] += 1
                right = parse_primary()
                return self._compare(value, op, right)
            elif (
                pos[0] < len(tokens)
                and tokens[pos[0]] == "is"
                and pos[0] + 1 < len(tokens)
                and tokens[pos[0] + 1] == "null"
            ):
                pos[0] += 2  # consume 'is null'
            ...
            if pos[0] < len(tokens):
                ...
                elif op == "is" and pos[0] + 1 < len(tokens) and tokens[pos[0] + 1] == "null":
                    pos[0] += 2
                    return left is None
        ...
        def parse_primary():
            ...
            # Handle field references (identifiers can contain underscores)
            elif (
                token.replace("_", "").isalnum()
                and token in context
                and token not in ["and", "or", "not", "is", "null"]
            ):
                pos[0] += 1
                return context[token]

            elif token.replace("_", "").isalnum() and token not in ["and", "or", "not", "is", "null"]:
                raise ValueError(f"Unknown field reference: {token}")
        ...
        try:
            ...
            logger.warning(f"Error evaluating expression '{self.expression}': {e}")
            return False
