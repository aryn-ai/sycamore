from sycamore.transforms.property_extraction.prompts import format_schema_v2, format_schema_v2_v2
...
complicated_schema_v2 = SchemaV2(
    properties=[
        NamedProperty(name="a", type=StringProperty(extraction_instructions="extract the letter a")),
        NamedProperty(
            name="b",
...
def test_format_schema_v2_v2():
    rpt = RichProperty.from_prediction({"f": "0123"})
    rpt.value["f"].invalid_guesses = ["0123"]
    f = format_schema_v2_v2(complicated_schema_v2, rpt)
    print(f)
    assert f == expected_formatted_schema_v2
