...
from sycamore.transforms.detr_partitioner import ArynPDFPartitioner, _supplement_text
...
from sycamore.transforms.text_extraction import get_text_extractor, PdfMinerExtractor, OcrModel
...
class TestArynPDFPartitioner:
    """Tests for ArynPDFPartitioner and related functionality.

    Tests that use ML models (DETR, table extraction) use the `use_fake_detr` fixture
    to return pre-recorded ground truth instead of running real inference.
    This makes unit tests fast while still testing the integration logic.
    """

    def test_supplement_text(self):
        """Test text supplementation logic (no ML models needed)."""
        infer1 = Element()
        infer2 = Element()
        infer3 = Element()
    ...
    def test_infer(self, use_fake_detr):
        """Test DETR inference using fake model with ground truth."""
        with Image.open(TEST_DIR / "resources/data/imgs/sample-detr-image.png") as image:
            d = FakeDeformableDetr("Aryn/deformable-detr-DocLayNet")
    ...
    def test_partition(self, use_fake_detr):
        """Test PDF partitioning using fake DETR model."""
    ...
    def test_partition_with_ocr_instance(self, use_fake_detr):
        """Test partitioning with custom OCR model instance."""
        s = ArynPDFPartitioner("Aryn/deformable-detr-DocLayNet")
    ...
    def test_table_extraction_order(self, use_fake_models):
    ...
    def test_detr_pdfminer_object_type(self, use_fake_models):
        """Test DETR with different pdfminer object types."""
        s = ArynPDFPartitioner("Aryn/deformable-detr-DocLayNet")
