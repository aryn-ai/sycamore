...
from sycamore.utils.bbox_sort import SortOptions, col_tag, find_overlap, bbox_sort_page
...
def test_page_basic_rtl() -> None:
    ...
    e4 = mkElem(0.15, 0.10, 0.85, 0.15)
    elems = [e0, e1, e2, e3, e4]
    bbox_sort_page(elems, sort_options=SortOptions(left_to_right=False))
...
def test_sort_elements_rtl_bbox() -> None:
    ...
    elems = [e1, e2, e3, e0]
    sort_elements(elems, mode="bbox", sort_options=SortOptions(left_to_right=False))
    answer = [e0, e2, e1, e3]
    assert elems == answer
    assert_element_index_sorted(elems)
...
def test_document_rtl_bbox() -> None:
    ...
    sort_document(doc, mode="bbox", sort_options=SortOptions(left_to_right=False))
    answer = [e0, e1, e2, e3]
    assert doc.elements == answer
    assert_element_index_sorted(doc.elements)
