...
@pytest.mark.parametrize(
    ...
    [
        (
            [(0., 0., 0., 0.), (1., 1., 1., 1.), (0.1, 0.2, 0.3, 0.4)],
            [(0., 0., 0., 0.), (1., 1., 1., 1.), (0.1, 0.2, 0.3, 0.4)],
        ),
        (
            [(0.1, 0.1, 0.9, 0.9)],
...
def test_margin_transform_page(original_bboxes: list[tuple[float, float, float, float]], expected_final_coordinates: list[tuple[float, float, float, float]]) -> None:
    ...
    for element, expected_bbox, original_bbox in zip(elements, expected_final_coordinates, original_bboxes):
        bbox = tuple(element.bbox.to_list())
