...
def pdf_to_image_files(pdf_path: str, file_dir: Path, fh = None, pages = None) -> Generator[Path, None, None]:
    ...
    if pdf_path == '-' and fh is None:
        raise ValueError("fh must be passed when pdf_path is set to '-'")

    if pages:
        logging.info(f"Processing selected pages {pages}")
    ...
    def read_stdout(fh, q):
        ...
        while True:
            ...
            if pages is None or (pages and (image_num + 1) in pages):
                if pages:
                    logging.info(f"Writing selected page {image_num + 1}")
    ...
    with LogTime("convert_to_image"):
        ...
        args = ["pdftoppm", "-r", "200", pdf_path]
        if pdf_path == '-':
            proc = Popen(args, stdin=fh, stdout=PIPE, stderr=PIPE)
        else:
            proc = Popen(args, stdout=PIPE, stderr=PIPE)
        q: Queue = Queue(1)
        t_out = Thread(
